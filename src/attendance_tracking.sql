...
CAST(COUNT (attendance.location_id) AS REAL) / COUNT(DISTINCT DATE(attendance.check_in_time)) AS avg_daily_attendance
FROM 
locations
LEFT JOIN
attendance ON locations.location_id = attendance.location_id
GROUP BY
locations.location_id, locations.name;
